...
void RotatableTetronimo::CyclicRoll(int &a, int &b, int &c, int &d)
{
   int temp = a;
   a = b;
   b = c;
...
void RotatableTetronimo::RotateLeft(){
/*n = _shape.size();
for(int i=0; i<n/2; i++)
   for(int j=0; j<(n+1)/2; j++)
       CyclicRoll(_shape[i][j], _shape[n-1-j][i], _shape[n-1-i][n-1-j], _shape[j][n-1-i]);
*/
}

void RotatableTetronimo::RotateRight(){
